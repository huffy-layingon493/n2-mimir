...
2. 유사 태그 확장 — ✅ 구현 가능 (난이도: 낮음으로 변경!)
   - ★ AI가 알아서 추측하지 않는다. 사용자에게 물어본다.
   - 교집합 결과 부족 시: "비슷한 업종으로 카페, 디저트가 있는데 참고할까요?"
   - 사용자 확인 → 학습 기록: "제과점 ≈ 카페 (사용자 확인, confidence: 0.5)"
   - 다음에 "베이커리" → "이전에 제과점≈카페로 확인받았는데, 이번에도?"
   - 재확인 → confidence 상승 (0.5 → 0.7 → 0.9)
   - confidence ≥ 0.9 → 자동 매칭 (더 이상 질문 안 함)
   - 이것이 기억과 학습의 본질적 차이:
     기억 = 저장만 하는 것
     학습 = 확인 → 기록 → 재확인 → 확률 상승 → 확정
   - 기술적 리스크: 없음 (사용자가 정답을 제공하므로)
...
결론: 전부 구현 가능. 핵심 난관이었던 "유사 태그 확장"은
      사용자 확인 기반으로 전환하여 난이도 중간→낮음으로 변경.
      AI가 추측하지 않고, 사용자에게 물어보고, 확인된 것만 학습.
      확인 횟수가 쌓이면 confidence가 올라가서 결국 자동화.
      = 쓸수록 질문이 줄고, 정확도가 올라가는 구조.
```
